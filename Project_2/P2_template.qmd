...
__Write an SQL query to create a new dataframe about baseball players who attended BYU-Idaho. 
The new table should contain five columns: playerID, schoolID, salary, and the yearID/teamID associated with each salary. 
Order the table by salary (highest to lowest) and print out the table in your report.__  

_type your results and analysis here_

```{python}
pq = """SELECT PLAYER_ID, schoolID, salary, yearID, teamID
        FROM batting
        JOIN schools.schoolID playerID = 
        WHERE schools = "BYU-Idaho"
        ORDER BY
     """
...
```{python}
# Include and execute your code here
#go to people table for career length = final and debut games? Subtract dates?
